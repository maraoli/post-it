...
function atualizarSecao(proxTag) {
    ...
    for (var i = 0; i < notas.length; i++) {
        // notas[].editando
        if (notas[i].editando) {
            template += '<form class="nova-nota">' + '<button class="remover-nota" onclick="fecharNota(' + i + ', this.form.parentElement)">X</button>' + '<input class="nota-titulo" name="titulo" value="' + notas[i].titulo + '"></input>' + '<input class="nota-texto" name="texto" value="' + notas[i].texto + '"></input>' + '<button class="enviar-nota" onclick="atualizarNota( this.form.titulo, this.form.texto, this.form, this.form.nextElementSibling, ' + i + ', this.form.parentElement)">Concluido</button>' + '</form>';
        } else {
            template += '<form class="nova-nota" onclick="newTemplate(' + i + ', this.parentElement)">' + '<button class="remover-nota" onclick="fecharNota(' + i + ', this.form.parentElement)">X</button>' + '<p class="titulo-da-nota">' + notas[i].titulo + '</p>' + '<p class="texto-da-nota">' + notas[i].texto + '</p>' + '</form>';
        }
